 ...
 *
 * Future stages (Security Agent, Performance Agent, etc.) can be
 * added via engine.addStage() without modifying existing stages.
 */
...
export class ReviewPipelineEngine {
  ...
  async execute(
    ...
    // Extract standards triggered from metadata
    ...
    stats.processingTimeMs = Date.now() - meta.startTime;
